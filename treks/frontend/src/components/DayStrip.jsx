...
export default function DayStrip({ days = [], selectedDate, onChange, size = 'md' }) {
  const pad = size === 'sm' ? 'px-2.5 py-2' : 'px-3 py-2 sm:px-3.5 sm:py-2'
  const minW = size === 'sm' ? 'min-w-[4rem]' : 'min-w-[4.25rem] sm:min-w-[5rem]'

  return (
    <div className="flex gap-2 overflow-x-auto no-scrollbar pb-0.5" role="group" aria-label="Pick a day">
      {days.map((day) => {
        ...
        return (
          <button
            ...
            className={`${minW} min-h-11 shrink-0 rounded-xl ${pad} text-center transition ${
              active
