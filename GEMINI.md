...
## AWS configuration for this project

- AWS CLI profile: `crwdctrl`. Pass `--profile crwdctrl` to AWS CLI commands.
- Selected project Region: `ap-south-1` (Mumbai).
- The Agent Toolkit catalog and AWS MCP endpoint use `us-east-1`; project resources use the selected project Region.
...
## Approved Claude Code Bedrock exception

The user approved global inference routing for Claude Code on Amazon Bedrock using the `crwdctrl` profile. Claude Code model requests may be processed outside Mumbai. This exception does not authorize an AWS plan upgrade or change the selected Region for project infrastructure. Confirm model access and credit eligibility before the first model invocation.
