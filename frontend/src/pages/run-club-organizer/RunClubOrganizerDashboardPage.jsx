...
export default function RunClubOrganizerDashboardPage() {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            setError('');
            try {
                const res = await fetchRunClubOrganizerDashboard(eventId);
                setData(res);
            } catch (e) {
                setError(e.message);
            } finally {
        ...
        <div className="space-y-6">
            <div>
                ...
                <p className="text-sm text-gray-500">
                    {event.city || '—'}
                    {event.distance ? ` · ${event.distance}` : ''} · Organizer dashboard
                </p>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                <StatCard label="Spots filled" value={stats.seatsFilled} icon={Users} accent="bg-[#0ECCEE]/15 text-[#0ECCEE]" />
                <StatCard label="Spots remaining" value={stats.seatsRemaining ?? '—'} icon={Clock} accent="bg-amber-500/15 text-amber-400" />
            ...
            <div className="flex flex-wrap gap-3">
                <Link to={`/run-club-organizer/events/${eventId}/participants`} className="px-4 py-3 min-h-[44px] inline-flex items-center rounded-xl bg-[#0ECCEE] text-black text-sm font-bold">View participants</Link>
                <Link to={`/run-club-organizer/events/${eventId}/scan`} className="px-4 py-3 min-h-[44px] inline-flex items-center rounded-xl border border-gray-700 text-sm font-medium hover:border-[#0ECCEE]/50">Open QR scanner</Link>
                <Link to={`/run-club-organizer/events/${eventId}/notifications`} className="px-4 py-3 min-h-[44px] inline-flex items-center gap-1.5 rounded-xl border border-gray-700 text-sm font-medium hover:border-[#0ECCEE]/50"><Bell size={16} /> Notify</Link>
            </div>
        </div>
    );
}
