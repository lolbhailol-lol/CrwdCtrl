                ...
                {loading ? (
                    ...
                    <div className="p-4 space-y-5">
                        ...
                        <div className="flex flex-wrap gap-2">
                            ...
                            <Badge tone="info">{participant.qrStatus}</Badge>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-3 text-sm">
                            <div>
                                <p className="text-[11px] text-gray-500 uppercase">Booking date</p>
                            ...
                            <div>
                                ...
                                <p className="text-emerald-400 font-medium">₹{Number(participant.organizerNet ?? participant.amountPaid ?? 0).toLocaleString('en-IN')}</p>
                            </div>
                            {(participant.platformFee ?? 0) > 0 ? (
                                <>
                                    <div>
                                        <p className="text-[11px] text-gray-500 uppercase">Platform fee (CrwdCtrl)</p>
                                        <p>₹{Number(participant.platformFee).toLocaleString('en-IN')}</p>
                                    </div>
                                    <div>
                                        <p className="text-[11px] text-gray-500 uppercase">Customer paid (total)</p>
                                        <p>₹{Number(participant.grossCollected ?? participant.amountPaid ?? 0).toLocaleString('en-IN')}</p>
                                    </div>
                                </>
                            ) : null}
                            {participant.bookingDetails?.paymentId ? (
                                <div>
                                    <p className="text-[11px] text-gray-500 uppercase">Payment ID</p>
                                    <p className="text-xs font-mono break-all">{participant.bookingDetails.paymentId}</p>
                                </div>
                            ) : null}
                        </div>

                        <div>
                            <p className="text-xs font-semibold text-gray-400 mb-2">Booking timeline</p>
                        ...
                        <button
                            ...
                            disabled={resending}
                            className="w-full py-2.5 rounded-xl border border-gray-700 text-sm font-medium hover:border-[#0ECCEE]/50 disabled:opacity-60"
                        >
