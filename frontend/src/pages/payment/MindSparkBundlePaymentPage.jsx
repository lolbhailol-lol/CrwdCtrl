...
export default function MindSparkBundlePaymentPage() {
  ...
  const pay = async () => { setBusy(true); setError(''); try { const result = await openCheckout(data); if (!result?.redirectDeferred) await refresh(); } catch (e) { setError(e.message); } finally { setBusy(false); } };
  const retry = async () => { setBusy(true); setError(''); try { const next = await reissueMindSparkBundlePayment(token); setData(next); if (!next.paymentSessionId) throw new Error('Could not prepare the payment. Please tap Retry payment again.'); const result = await openCheckout(next); if (!result?.redirectDeferred) await refresh(); } catch(e) { setError(e.message); } finally { setBusy(false); } };
        ...
        {data ? (
          <>
            ...
            {data.status === 'paid' ? (
              ...
              <button disabled={!data.paymentSessionId || busy} onClick={pay} className="w-full rounded-xl bg-[#0ECCEE] py-3 font-bold text-black disabled:opacity-50 inline-flex items-center justify-center gap-2">
                {busy ? <Loader size={18} className="animate-spin" /> : null}
                {busy ? 'Opening secure payment…' : 'Pay securely with Cashfree'}
              </button>
            )}
          </>
        ) : null}
