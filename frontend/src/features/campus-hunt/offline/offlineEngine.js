...
export function markReachedStart(bundle, session, state, finishCode = '', now = new Date()) {
  ...
  if (next.currentStage === 'CLUE_6_ACTIVE' && canTransition(next.currentStage, 'CLUE_6_COMPLETED')) {
    ...
    row.state = 'COMPLETED';
    // Place points (200, 190, … 10) are set on the live board when this phone syncs.
    row.awardedPoints = 0;
    row.completedAt = now.toISOString();
  ...
  return {
    state: next,
    meta: {
      message: 'Finish code accepted. First team in gets 200, then −10 each, down to 10. Score locked.',
      awardedPoints: 0,
      finalScore: next.score,
      scoreLocked: true,
    },
