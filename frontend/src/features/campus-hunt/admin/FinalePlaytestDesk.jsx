  ...
  return (
    <section className="rounded-2xl border border-[#0ECCEE]/35 bg-[#0a1218] p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-white/55">
            Select finalist → Release → open link → cheat missions → Mark finish
          </p>
        </div>
        {roundStatus && (
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-3">
        <button
          type="button"
          disabled={Boolean(busy) || !teamId || completed.includes('intel_hunt')}
          onClick={() => run(
            'intel',
        ...
        >
          ...
          <p className="text-sm font-bold text-orange-50">
            {busy === 'intel' ? '…' : completed.includes('intel_hunt') ? 'Intel done ✓' : 'Complete Intel Hunt'}
          </p>
        ...
        <button
          type="button"
          disabled={Boolean(busy) || !teamId || completed.includes('field_terminal') || completed.includes('borrowed_device')}
          onClick={() => run(
            'grid',
            () => adminPlaytestCompleteFinaleMission(eventId, teamId, 'field_terminal'),
            'Field Terminal forced complete (+100)',
          )}
        ...
        >
          ...
          <p className="text-sm font-bold text-blue-50">
            {busy === 'grid' ? '…' : completed.includes('field_terminal') || completed.includes('borrowed_device') ? 'Terminal done ✓' : 'Complete Field Terminal'}
          </p>
          <p className="mt-0.5 text-[11px] text-blue-100/50">Skip grid · +100</p>
        </button>
