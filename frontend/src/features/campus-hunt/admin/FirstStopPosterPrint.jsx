...
const THEME = STAGE_THEMES.clue1;

/**
 * Clue 1 print: 40 pocket-size yellow cards (10 places × 4) — tuck anywhere, not wall posters.
 */
export default function FirstStopPosterPrint({ eventId, reloadKey = 0 }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setPacks(result.data?.firstStopPrintPacks || []);
      setSummary(result.data?.printSummary || null);
    } catch (err) {
      setError(err.message || 'Could not load Clue 1 posters');
    } finally {
      setBusy(false);
    }
  }, [eventId]);
  ...
  const printPacks = async (selectedPacks) => {
    ...
    for (const pack of selectedPacks) {
      ...
      const cards = await Promise.all(posters.map(async (poster) => {
        ...
        return `
          <article class="card">
            <p class="badge">Yellow · FIRST SCAN</p>
            ...
            <p class="note">All 4 scan → Clue 2 · then take this card</p>
          </article>
        `;
      }));
      sheets.push(`
        <section class="sheet">
          <div class="sheet-head">
            <div>
              <p class="badge">Clue 1 · Yellow · pocket cards</p>
    ...
    popup.document.write(`<!doctype html><html><head><title>Clue 1 · Yellow · pocket cards</title>
    ...
    setMessage(`Print sheet opened · ${count} yellow pocket cards`);
  };
    ...
    <section className={`rounded-2xl border ${THEME.borderClass} ${THEME.bgClass} p-4`}>
        ...
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-white">
              Clue 1 pocket cards · {TARGET_POSTERS} total
            </h2>
          </div>
          <p className="mt-1 text-xs text-white/50">
            Small yellow FIRST SCAN cards — cut &amp; tuck anywhere (not wall posters).
            Teams must pick up their card after scanning so later teams only find theirs.
          </p>
        </div>
      ...
      {stats.posterCount < TARGET_POSTERS && (
        <p className="mt-2 text-xs text-amber-200">
          Need {TARGET_POSTERS - stats.posterCount} more — Save Clue 1 to bind teams, then refresh.
        </p>
      )}
