...
/**
 * Clue 3 / Checkpoint 3 print: 40 pocket-size blue THIRD SCAN cards (10 × 4).
 */
export default function ThirdStopPosterPrint({ eventId, reloadKey = 0 }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setPacks(result.data?.thirdStopPrintPacks || []);
      setSummary(result.data?.printSummary || null);
    } catch (err) {
      setError(err.message || 'Could not load THIRD SCAN posters');
    } finally {
      setBusy(false);
    }
  }, [eventId]);
  ...
  const printPacks = async (selectedPacks) => {
    ...
    for (const pack of selectedPacks) {
      ...
      const cards = await Promise.all(posters.map(async (poster) => {
        ...
        return `
          <article class="card">
            <p class="badge">Blue · THIRD SCAN</p>
            ...
            <p class="note">All 4 scan → riddle · then take card</p>
          </article>
        `;
      }));
      sheets.push(`
        <section class="sheet">
          <div class="sheet-head">
            <div>
              <p class="badge">Clue 3 · Blue · pocket cards</p>
    ...
    popup.document.write(`<!doctype html><html><head><title>Clue 3 · Blue · pocket cards</title>
    ...
    setMessage(`Print sheet opened · ${count} Blue pocket cards`);
  };
    ...
    <section className={`rounded-2xl border ${THEME.borderClass} ${THEME.bgClass} p-4`}>
        ...
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-white">
              THIRD SCAN pocket cards · {TARGET_POSTERS} total
            </h2>
          </div>
          <p className="mt-1 text-xs text-white/50">
            Small blue cards — tuck anywhere. Finding them early does nothing until after Clue 3 riddle.
            After scanning, teams pick up their card so later teams only find theirs.
          </p>
        </div>
      ...
      {stats.posterCount < TARGET_POSTERS && (
        <p className="mt-2 text-xs text-amber-200">
          Need {TARGET_POSTERS - stats.posterCount} more — Bootstrap + Save Clue 3, then refresh.
        </p>
      )}
