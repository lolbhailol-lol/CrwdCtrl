...
function SportsFestPage() {
    ...
    // Function to fetch sports fests
    const fetchSportsFests = async () => {
        try {
            setLoading(true);
            setError(null);
            ...
            // Filter for sports fests only and exclude last year hits
            const sportsFests = festsList.filter(fest => 
                fest.festType === 'sports' && fest.status !== 'lastyearhit'
            );
            
            // Transform data to match expected structure
            const transformedFests = sportsFests.map(fest => ({
                id: fest._id || fest.id,
                title: fest.festName,
            ...
            console.error('Error fetching sports fests:', err);
            setError('Failed to load sports fests');
            setFests([]);
        } finally {
            setLoading(false);
        }
    ...
    // Fetch sports fests on component mount
    useEffect(() => {
        fetchSportsFests();
    }, []);
    ...
    useEffect(() => {
        const handleAdminUpdate = () => {
            console.log('🔄 Admin update detected - refetching sports fests');
            fetchSportsFests();
        };
        ...
        const handleStorageChange = (e) => {
            if (e.key === 'admin_data_updated') {
                console.log('🔄 Admin update detected (cross-tab) - refetching sports fests');
                fetchSportsFests();
            }
        };
        ...
        return (
            <div className={`min-h-screen flex transition-colors ${isDark ? 'bg-dark-950' : 'bg-white'}`}>
                <div className="flex flex-1 flex-col items-center justify-center">
                    ...
                    <h2 className={`text-xl font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>Loading sports fests...</h2>
                </div>
            </div>
        );
    }
            ...
            <div className={`flex flex-1 flex-col transition-all duration-300 ${isProfileOpen ? 'blur-sm' : ''}`}>
                ...
                <div className="md:hidden flex items-center justify-between p-4 pb-2">
                    <div className="flex items-center">
                        ...
                        <h1 className={`text-xl font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>Sports Fests</h1>
                    </div>
                    <button
                ...
                <main className="flex-1 p-4 sm:p-6 md:pt-6 pt-2">
                    ...
                    <h1 className={`hidden md:block text-xl sm:text-2xl font-semibold mb-6 sm:mb-8 ${isDark ? 'text-white' : 'text-gray-900'}`}>Sports Fests</h1>
